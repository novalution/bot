...
const misuh = () => new Promise((resolve, reject) => {
  let arr = [
    'asu',
    'bajingan',
    'jembut',
...
const misuhdowo = () => new Promise((resolve, reject) => {
  let kata = [
    'asu',
    'bajingan',
    'jembut',
  ...
  let randKata = kata[Math.floor(Math.random() * kata.length)]
  let list = [
    `rupamu koyo ${randKata}`, `gelut pe ${randKata} !!!`,
    `wey ${randKata}!!`,`ga seneng send lokasi lo ${randKata}`,
    `kesel nyawang dapuranmu ${randKata}`, `aku mudeng nek koe ki ${randKata}`,
    `tonjok ndasmu pe ${randKata}`
   ]
   resolve(list[Math.floor(Math.random() * list.length)])
})
